...
function startAdminAutoRefresh() {
  ...
  const isAdmin =
    localStorage.getItem("isAdmin") === "true" || email === "admin@admin.com";
...
function renderMoviesByGenre(selectedGenre) {
  ...
  const genreFiltered = moviesData.filter((movie) => {
    const genres = Array.isArray(movie.genre) ? movie.genre : [movie.genre];
    return genres.some(
      (g) => String(g).toLowerCase() === String(selectedGenre).toLowerCase()
    );
  });
  const mode = (
    document.getElementById("sortSelect")?.value ||
  ...
  const track = categoryRow.querySelector(".carousel-track");
  const infiniteMovies =
    sorted.length >= 4 ? [...sorted, ...sorted, ...sorted] : sorted;
  infiniteMovies.forEach((movie) => {
...
function createMovieCard(movie) {
  ...
  const genreDisplay = Array.isArray(movie.genre)
    ? movie.genre.join(", ")
    : movie.genre;
  const firstGenre = Array.isArray(movie.genre)
    ? movie.genre[0] || ""
    : movie.genre || "";
  movieCard.innerHTML = `
    <div class="movie-poster">
      ...
      <div class="card-actions">
        <button class="action-btn play" data-id="${
          movie.id || movie._id
        }"><i class="fas fa-play"></i></button>
        <button class="action-btn add"><i class="fas fa-plus"></i></button>
        <button class="action-btn like ${
          userHasLiked ? "active" : ""
        }" data-like-id="${movie._id}"><i class="${
    userHasLiked ? "fas" : "far"
    ...
    <div class="movie-info">
      ...
      <button class="like-btn ${userHasLiked ? "liked" : ""}" data-movie-id="${
    movie._id
  }">
...
function setupCarousel(categoryIndex, originalLength) {
  ...
  const leftBtn = document.querySelector(
    `.carousel-arrow.left[data-category="${categoryIndex}"]`
  );
  const rightBtn = document.querySelector(
    `.carousel-arrow.right[data-category="${categoryIndex}"]`
  );
...
function updateLikeButton(movieId, likeCount, userHasLiked) {
  ...
  buttons.forEach((button) => {
    ...
    button.innerHTML = `<i class="heart-icon ${
      userHasLiked ? "fas" : "far"
    } fa-heart"></i> ${likeCount} ${likeCount === 1 ? "like" : "likes"}`;
...
function showSearchResults(query, results) {
  ...
  if (results.length === 0) {
    searchGrid.innerHTML =
      '<div style="color: white; text-align: center; width: 100%; margin-top: 50px;">No results found</div>';
    return;
  }
...
function openMoreLikeThis(genre) {
  ...
  const results = moviesData.filter((m) => {
    ...
    return gs.some(
      (g) => String(g).toLowerCase() === String(genre).toLowerCase()
    );
  });
  results.forEach((m) => {
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  if (modalClose)
    modalClose.addEventListener("click", () => closeMoreLikeThis());
