...
function populateGenresDropdown() {
  ...
  document
    .querySelectorAll("#genreFilterMenu .dropdown-item")
    .forEach((item) => {
...
function renderMovies(filterType = null) {
  ...
  if (filterType === "movie") {
    filteredData = filteredData.filter(
      (movie) => movie.type.toLowerCase() === "movie"
    );
  } else if (filterType === "show") {
    filteredData = filteredData.filter(
      (movie) => movie.type.toLowerCase() === "series"
    );
  }
...
function createMovieCard(movie) {
  ...
  movieCard.innerHTML = `
        <div class="movie-poster">
            <img src="${movie.thumbnail || movie.poster}" alt="${
    movie.title
  } poster">
        </div>
